<?php
class Prefacturacion_automatica extends CI_Controller
{
	...
	public function index($idcliente=0,$idempresa=0)
	{
		...
		$body=$this->load->view('prefacturacionautomatica/index',array(
			"menumain"=>$menumain,
				...
				switch(strtoupper($hoja->getAttribute("nombre")))
				{
					case "CLIENTES":
						foreach($hoja->getElementsByTagName("fila") as $fila)
						{
							...
							//var_dump( $objeto, $clientes, $auxnoctetmp, $clientes[$auxnoctetmp] );
						}
						break;
					case "GENERADORES":
						foreach($hoja->getElementsByTagName("fila") as $fila)
						{
							...
							if($fac->getIdfacturacion()==0||$fac->getIdfacturacion()=="")
								...
								$objeto->setFacturaciones($fac->getIdfacturacion());
							//var_dump( $clientes, $auxnoctetmp, $clientes[$auxnoctetmp] );
